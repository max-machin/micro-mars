...
const CardProduct = ({ product, onCardClick, isDetail = false, elementStyle = {}}) => {
    ...
    return (    
        <div className="card background-element" ref={cardRef} onClick={(e) => {isDetail === false ? onCardClick(handleClick(e)) : null}} style={elementStyle} >
            <img className={isDetail ? "img-produc-detail-open" : "img-product"} src={product.product_picture} alt="Image du produit" />
            <div className={isDetail ? "card-content-detail-open" :"card-content"} elementIsdetail={isDetail}>
            {!isDetail && 
            <>
                <div className = "card-header">
                    <img className="img-exponent" src={product.exponent_picture} alt="Image de l'exposant" />
                    <h3>{product.product_name}</h3>
                </div>
                <span>{product.price} €</span>
                <hr />
                <span className="exponent-name">Exposé par {product.exponent_name}</span>
                </>
            }
            </div>
            {isDetail && 
            <>
                    <div className="detail-card-open-modale">
                        <img className= "img-product-detail" src={product.product_picture} alt="Image du produit" />
